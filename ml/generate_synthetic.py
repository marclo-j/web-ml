...
COLUMNAS_BASE = [
    "codigo", "grado", "seccion", "grupo", "momento",
    "suma_notas", "n_notas",
    "dias_asistidos", "dias_programados",
    "reuniones_asistidas", "reuniones_programadas",
]


def generar_lote(n, anio, rng, grupo_fijo, momento, incluir_deserto, prefijo):
    filas = []
    for i in range(n):
        codigo = f"{prefijo}-{anio}-{i + 1:04d}"
        grado = int(rng.choice([3, 4]))
        seccion = str(rng.choice(["A", "B"]))
        grupo = grupo_fijo if grupo_fijo else str(rng.choice(["control", "experimental"]))

        # Rendimiento: escala vigesimal (0-20), entre 8 y 12 cursos evaluados
        n_notas = int(rng.integers(8, 13))
        promedio_base = float(np.clip(rng.normal(13.5, 3.0), 5, 20))
        suma_notas = round(promedio_base * n_notas, 1)

        # Asistencia: entre 180 y 200 días programados en el periodo
        dias_programados = int(rng.integers(180, 201))
        pct_asist_base = float(np.clip(rng.normal(0.85, 0.15), 0.4, 1.0))
        dias_asistidos = int(round(pct_asist_base * dias_programados))

        # Apoyo familiar: entre 3 y 5 reuniones programadas
        reuniones_programadas = int(rng.integers(3, 6))
        pct_reun_base = float(np.clip(rng.normal(0.5, 0.3), 0.0, 1.0))
        reuniones_asistidas = int(round(pct_reun_base * reuniones_programadas))

        fila = {
            "codigo": codigo, "grado": grado, "seccion": seccion, "grupo": grupo,
            "momento": momento,
            "suma_notas": suma_notas, "n_notas": n_notas,
            "dias_asistidos": dias_asistidos, "dias_programados": dias_programados,
        ...
        if incluir_deserto:
            ...
            pct_reuniones = (
                ...
                if reuniones_programadas > 0 else 0.0
            )
...
def main():
    ...
    parser.add_argument("--prueba", type=int, default=20,
    ...
    hist_2024 = generar_lote(mitad, 2024, rng, grupo_fijo=None, momento="pre",
                              incluir_deserto=True, prefijo="HIST")
    hist_2025 = generar_lote(args.historico - mitad, 2025, rng, grupo_fijo=None,
                              momento="pre", incluir_deserto=True, prefijo="HIST")
    historico = pd.concat([hist_2024, hist_2025], ignore_index=True)

    prueba = generar_lote(args.prueba, 2026, rng, grupo_fijo=None, momento="pre",
                           incluir_deserto=False, prefijo="EST")
    ...
    print(f"[OK] {ruta_hist} -> {len(historico)} filas | tasa de deserción sintética: {tasa:.1f}%")
    print(f"[OK] {ruta_prueba} -> {len(prueba)} filas (sin 'deserto', formato de carga masiva)")
